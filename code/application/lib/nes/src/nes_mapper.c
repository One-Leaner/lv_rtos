...
static inline uint16_t nes_cache_tick(nes_rom_info_t *rom)
{
    ...
    if (rom->cache_tick == 0)
    {
        ...
        for (int i = 0; i < NES_PRG_CACHE_SLOTS; i++)
            rom->prg_cache[i].last_used = 0;
        for (int i = 0; i < NES_CHR_CACHE_SLOTS; i++)
            rom->chr_cache[i].last_used = 0;
        rom->cache_tick = 1;
    }
...
static inline uint8_t *nes_prg_cache_get(nes_t *nes, uint16_t src)
{
    ...
    for (int i = 0; i < NES_PRG_CACHE_SLOTS; i++)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            if (nes->nes_cpu.prg_banks[j] == buf)
            {
                active = 1;
                break;
            }
...
static inline uint8_t *nes_chr_cache_get(nes_t *nes, uint16_t src)
{
    ...
    for (int i = 0; i < NES_CHR_CACHE_SLOTS; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            if (nes->nes_ppu.pattern_table[j] == buf)
            {
                active = 1;
                break;
            }
...
#define NES_CASE_LOAD_MAPPER(mapper_id) \
    case mapper_id:                     \
        return nes_mapper##mapper_id##_init(nes)
